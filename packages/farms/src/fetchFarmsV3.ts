...
// import { getAllPancakeBunniesLowestPrice } from 'state/nftMarket/helpers'

// export const PriceCakeUSD = (): BigNumber {
//   const cake = useCakePriceAsBN()
//   console.log("hereaaa", cake)
//   return cake
// }
export async function farmV3FetchFarms({
  ...
  const farmsWithPrice = getFarmsPrices(farmsData, cakePrice.price?.toString(), combinedCommonPrice)
...
export function getFarmsPrices(
  ...
  const commonPriceFarms = farms.map((farm) => {
    ...
    } else if (farm.quoteToken.symbol === 'USDC') {
      symbol = '0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48';
    }
    // try to get price via common price
    if (commonPrice[symbol]) {
      quoteTokenPriceBusd = new BN(commonPrice[symbol])
    }
    
    if (farm.token.symbol === 'WETH') {
      symbol = '0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2';
      
    } else if (farm.token.symbol === 'USDC') {
      symbol = '0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48';
    ...
    if (commonPrice[symbol]) {
      tokenPriceBusd = new BN(commonPrice[symbol])
    }
    // try price via CAKE
    if (
